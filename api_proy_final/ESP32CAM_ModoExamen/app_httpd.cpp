...
static esp_err_t win_handler(httpd_req_t *req) {
  ...
  log_i(
    "Set Window: Start: %d %d, End: %d %d, Offset: %d %d, Total: %d %d, Output: %d %d, Scale: %u, Binning: %u", startX, startY, endX, endY, offsetX, offsetY,totalX, totalY, outputX, outputY, scale, binning);
sensor_t *s = esp_camera_sensor_get();
...
// Handler para controlar el LED desde Python/HTTP
...
if (httpd_req_get_url_query_str(req, param, sizeof(param)) == ESP_OK) {
    ...
    if (httpd_query_key_value(param, "state", value, sizeof(value)) == ESP_OK) {
        ...
        // Controla el LED en GPIO 4
        gpio_set_level((gpio_num_t)4, state);

        httpd_resp_set_type(req, "text/plain");
        httpd_resp_send(req, state ? "LED ON" : "LED OFF", HTTPD_RESP_USE_STRLEN);
        return ESP_OK;
    }
}
...
// Configura GPIO 4 como salida para control manual del LED
